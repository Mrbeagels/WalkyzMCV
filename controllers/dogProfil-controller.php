...
//     // Avec une regex (constante déclarée plus haut), on vérifie si c'est le format attendu 
//     if (!$testRegex) {
//         $error["weight"] = "Le poid n'est pas au bon format!!";
//     }  else { // Pour les champs obligatoires, on retourne une erreur
//     $error["weight"] = "Vous devez choisir un poid valide!!";
// }
// }
...
if (!empty($behavior)) {
    ...
    // var_dump($name, $nickname, $birthdate, $weight, $breed, $stats, $behavior, $description);
    if (empty($error)) {
        // **HYDRATATION **/
        $dog = new Dog_profil;
        $dog->setName($name);
